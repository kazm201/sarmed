...
export const AuthProvider = ({ children }) => {
  ...
  const updateSettings = (changes) => {
    ...
    setSettings(updated); localStorage.setItem(SETTINGS_STORAGE_KEY, JSON.stringify(updated));
  };
  const login = (username, password) => {
    const cleanUsername = username.trim(); const cleanPassword = password.trim();
    ...
    if (cleanUsername === settings.managerUsername && cleanPassword === settings.managerPassword) {
      ...
      setCurrentUser(user); localStorage.setItem(AUTH_STORAGE_KEY, JSON.stringify(user)); return { success: true };
    }
  ...
  return <AuthContext.Provider value={{ currentUser, settings, updateSettings, login, logout, isManager: currentUser?.role === 'manager', isWorker: currentUser?.role === 'worker' }}>{children}</AuthContext.Provider>;
};
