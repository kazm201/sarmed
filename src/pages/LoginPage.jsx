  ...
  return <div className="min-h-screen w-full flex items-center justify-center p-4 bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950/40">
    <div className="w-full max-w-md glass-panel rounded-3xl p-6 sm:p-8 shadow-2xl border border-slate-700/60">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div><label className="block text-xs font-bold text-slate-300 mb-2">اسم المستخدم</label><div className="relative"><User className="absolute right-3.5 top-3.5 w-4 h-4 text-slate-400" /><input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="أدخل اسم المستخدم" autoComplete="username" className="w-full pr-10 pl-4 py-3 rounded-2xl glass-input text-sm font-semibold" required autoFocus /></div><p className="text-[11px] text-slate-500 mt-1.5">استخدم بيانات الحساب التي أنشأها صاحب الماركت.</p></div>
